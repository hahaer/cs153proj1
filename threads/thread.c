...
thread_tick (void) 
{
  ...
  if(!(ptr == list_end(&sleep_list)))
	{	
		for(;ptr != NULL && ptr->prev != NULL && ptr->next != NULL;)
		{	
			ptr->counter--;
			ptr = list_next(ptr);
		}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  t->magic = THREAD_MAGIC;
	t->held_by_lock = false;
	t->set_priority_while_locked = -1;
  ...
  if (list_empty (&ready_list))
  ...
  else
	{
	  struct list_elem * high = list_begin(&ready_list);
		...
		ready_list_ptr = list_next(ready_list_ptr))
		{
			if(list_entry(ready_list_ptr, struct thread, elem)->priority >
			list_entry(high, struct thread, elem)->priority)
				high = ready_list_ptr;
		}
		struct thread * new_current = list_entry(high, struct thread, elem);
		list_remove(high);
		return new_current;
